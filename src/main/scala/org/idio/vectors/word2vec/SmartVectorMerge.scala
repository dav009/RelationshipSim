package scala.org.idio.vectors.word2vec
 ...
 */
class SmartVectorMerge(vectors: List[Word2VecVector]) {
  ...
  def merge(): Word2VecVector = {
    this.mergeVectorsPickDimensions(vectors)
  }
